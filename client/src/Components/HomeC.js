...
export default class Home extends Component {
  ...
  exportPDF = () => {
    ...
    const data = this.state.posts.map((elt) => [
      elt.stockId,
      elt.stockType,
      elt.brand,
      elt.category,
  ...
  // Retrieve stock data (with auth header if needed)
  retrievePosts() {
    const token = localStorage.getItem("authToken"); // Get token from localStorage

    // Pass the token in headers if it's available
    axios
  ...
  render() {
    return (
      <div className="container">
        ...
        <div className="row">
          <div className="col-lg-9 mt-2 mb-2">
            <button className="btn btn-outline-primary">
              <a href="/add" style={{ textDecoration: "none", color: "blue" }}>
                <i className="fa fa-plus"></i>&nbsp;Create New Stock
              </a>
            </button>{" "}
            &nbsp;
        ...
        >
          <table
            className="table shadowBox shadow p-3 mb-5 bg-white rounded"
